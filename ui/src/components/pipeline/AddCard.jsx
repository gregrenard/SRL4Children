...
export const AddCard = ({ label, onClick, icon, description }) => {
  ...
  const iconKey = icon || (
    ...
    label.toLowerCase().includes('attack') ? 'attack' :
    label.toLowerCase().includes('score') ? 'score' :
    label.toLowerCase().includes('rule') || label.toLowerCase().includes('guardrail') ? 'guardrail' :
    null
  );

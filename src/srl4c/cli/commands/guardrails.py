...
def generate_guardrails_cmd(console: Console, score_id: str, max_rules: int = 3, max_total: int = 20):
    ...
    from srl4c.core.guardrails import create_guardrails, get_guardrails_details, load_guardrails_config, run_guardrails
    ...
    if not details:
        console.print("[red]Error: Could not get guardrail set details[/red]")
        return
    ...
    for idx, g in enumerate(all_guardrails, 1):
        ...
        table.add_row(str(idx), short_p, g["rule_text"][:70] + "..." if len(g["rule_text"]) > 70 else g["rule_text"])

    console.print(table)
    console.print()
    console.print("Next steps:")
    console.print("  [cyan]srl4c guardrails list[/cyan]           # View all guardrail sets")
...
def generate_worker(console: Console, set_id: str, output_path: str = None):
    ...
    worker_code = WORKER_TEMPLATE.format(
        set_id=gset["id"][:8], rules_count=len(guardrails), guardrails_text=guardrails_text
    )
...
def deploy_guardrails(console: Console, set_id: str):
    ...
    # Check login
    try:
        result = subprocess.run(wrangler_cmd + ["whoami"], capture_output=True, text=True, timeout=10)
...
""")
        ...
        try:
            result = subprocess.run(wrangler_cmd + ["deploy"], cwd=tmpdir, capture_output=True, text=True, timeout=60)

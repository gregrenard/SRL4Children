...
def run_score(
    ...
    from srl4c.core.score import (
        create_score,
        get_score_details,
    )
    from srl4c.core.score import (
        run_score as execute_score,
    )
    ...
    console.print(
        f"  Judges: [cyan]{len(judge_config.judges)}[/cyan] ({', '.join(j.name for j in judge_config.judges)})"
    )
...
def display_score_summary(
    ...
    console.print()
    console.print(
        f"Final Aggregate Score: [bold]{final_score:.1f} / 5.0[/bold]  "

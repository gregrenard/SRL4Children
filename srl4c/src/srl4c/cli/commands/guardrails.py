...
def load_guardrails_config() -> Dict[str, Any]:
    ...
    user_config = Path.home() / ".srl4c" / "guardrails.yaml"
    template_config = SRL4C_ROOT / "templates" / "guardrails.yaml"
...
def load_criterion_spec(criterion_id: str) -> str:
    """Load criterion specification from .prompt file (like existing code)"""
    parts = criterion_id.split('.')
    assets_root = Path(__file__).parent.parent.parent.parent.parent.parent / "assets" / "criteria"

    # Try to find the .prompt file
    search_path = assets_root.joinpath(*parts[:-1]) if len(parts) > 1 else assets_root
